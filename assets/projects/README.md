...
Each file exposes `#project-icon` with `fill="none"` and `stroke="currentColor"`. The portfolio controls the inherited color through `.project-image { --project-icon-color: var(--accent); }` and `.project-icon { color: var(--project-icon-color); }`. All five use the same blue accent as the rest of the portfolio (#5BAFD6).
